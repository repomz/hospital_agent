...
"""
DICOM PACS → Yandex Cloud Transfer
Использует проверенную логику скачивания из первого скрипта
+ параллельная отправка в Yandex Cloud
"""
...
# ==============================
# PACS CLIENT (ПОЛНОСТЬЮ ИЗ ПЕРВОГО ФАЙЛА)
# ==============================
...
class PACSClient:
    ...
    def _build_store_handler(self, study_dir, expected_instances, upload_queue=None):
        ...
        def handle_store(event):
            ...
            # Если есть очередь для загрузки в Yandex, добавляем файл
    ...
    # ==========================
    # GET (МОДИФИЦИРОВАН ДЛЯ YANDEX CLOUD)
    # ==========================
    def get(self, study_uid, upload_to_yandex=False, debug=False):
        ...
        # Инициализация Yandex Cloud если нужно
        upload_queue = None
        upload_thread = None
        yandex_folder = None
        files_uploaded = 0
        ...
        if upload_to_yandex:
            try:
                ...
                # Формируем имя папки в Yandex Cloud
                if patient_name:
                    # Форматируем имя как в первом скрипте: Petrov_AV_10.03.2026
                    parts = str(patient_name).split("^")
                    ...
                    patient_folder = surname
                    if initials:
                        patient_folder = f"{surname}_{initials}"
                    ...
                    yandex_folder = f"{patient_folder}_{date_formatted}"
                else:
                    yandex_folder = f"Unknown_{datetime.now().strftime('%d.%m.%Y')}"
                
                logger.info(f"Папка в Yandex Cloud: {yandex_folder}")
                
                # Создаем очередь для файлов
                upload_queue = queue.Queue(maxsize=50)
                
                # Функция для загрузки в Yandex
                def upload_worker():
                    """Фоново загружает сохраненные DICOM-файлы в Yandex Cloud."""
                    nonlocal files_uploaded, bytes_uploaded
                    logger.info("Поток загрузки в Yandex Cloud запущен")
                    
                    while True:
                        try:
                            file_info = upload_queue.get(timeout=2)
                            if file_info is None:  # Сигнал остановки
                                break
                            ...
                            logger.debug(f"↑ Загружен {Path(file_path).name} ({file_size/1024:.1f} KB) за {upload_time:.2f}с")
                            
                            # Не удаляем файл здесь, он понадобится если retry
                    ...
                    logger.info(f"Поток загрузки завершен. Загружено: {files_uploaded} файлов")
                ...
                logger.error(f"Ошибка инициализации Yandex Cloud: {e}")
        ...
        handlers = [
            (evt.EVT_C_STORE, self._build_store_handler(study_dir, expected_instances, upload_queue))
        ]
        ...
        # Завершаем поток загрузки в Yandex
        if upload_thread and upload_queue:
            logger.info("Ожидание завершения загрузки в Yandex Cloud...")
            upload_queue.put(None)  # Сигнал остановки
        ...
        logger.info("=" * 60)
        logger.info("ИТОГИ ПЕРЕДАЧИ:")
        logger.info(f"  Скачано из PACS: {self.received} файлов ({self.received_bytes/(1024*1024):.2f} MB)")
        
        if upload_to_yandex and files_uploaded > 0:
            logger.info(f"  Загружено в Yandex: {files_uploaded} файлов ({bytes_uploaded/(1024*1024):.2f} MB)")
            logger.info(f"  Папка в бакете: {yandex_folder}")
        elif upload_to_yandex:
            logger.warning("  В Yandex Cloud не загружено ни одного файла")
        
        logger.info(f"  Время выполнения: {time.time() - self.study_start_time:.2f} сек")
        logger.info("=" * 60)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="DICOM PACS CLI with Yandex Cloud upload")
    ...
    get_parser.add_argument("--yandex", action="store_true", help="Загрузить в Yandex Cloud")
