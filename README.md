...
Единый Python-проект для постоянного агента больничного компьютера: heartbeat в backend, PACS polling, отправка протоколов операций, выполнение команд из `viewer_url/user_requests`, скачивание DICOM, отправка в Yandex Cloud и почасовая отправка логов. Импорт из Yandex в remote PACS и формирование отчётов выполняет backend.
...
├── scripts/                    # импорт архива, аудит и статистика
└── tests/                      # автоматические проверки
```

## Установка

### Вариант 1: установка через requirements.txt
...
Зависимости описаны только в `pyproject.toml`; `requirements.txt` устанавливает
тот же проект, чтобы два списка зависимостей не расходились.
...
GitHub Actions выполняет эти проверки на Windows и Linux с Python 3.10 и 3.13.
Тесты используют временные файлы и подменяют сетевые вызовы: запускать рабочий
агент для проверки кода не требуется. Конфигурации, реквизиты и локальное
состояние не входят в устанавливаемый Python-пакет.
...
Поле `viewer_url` — это единый внешний URL viewer backend. Для действующего
сервера используется `https://135.106.195.161/api`; прямые внешние адреса портов
8080 и 8042 агенту не нужны.
...
- `services/operation_reports.py` — общий парсинг DOCX для протоколов и скриптов импорта; прежние вспомогательные функции отчётов сохранены для совместимости, но рабочий агент отчёты не формирует;
...
- загружает DICOM в Yandex ограниченно-параллельно; число потоков можно изменить
  переменной `YANDEX_UPLOAD_WORKERS` (по умолчанию 6).

Пример `agent_config.json`:

```json
{
  "viewer_url": "https://135.106.195.161/api",
  "agent_id": 2,
...
Завершённые часы логов отправляются при запуске (если ещё не отправлены), затем
в начале каждого календарного часа: 22:00, 23:00 и т. д. Диспетчер проверяет
задачи с шагом около секунды; при недоступности сети фактическая доставка может
задержаться. Текущий незавершённый час не отправляется.

Heartbeat также передаёт безопасный снимок настроек polling, папки протоколов,
версию и описание агента. Backend хранит последний снимок; посмотреть его можно
только администратору в разделе «Настройки». Пароли и ключи Yandex не передаются.
Для этой функции требуется backend с миграцией `20260926000000_agent_configuration`.
...
Перед автоматической передачей XA агент сверяет возвращаемые PACS количества
серий и экземпляров. Исследование должно показывать неизменные ненулевые
значения не менее 20 минут; если PACS продолжает принимать кадры, окно
стабилизации начинается заново. Ручная загрузка по запросу пользователя этой
задержкой не ограничивается и по-прежнему проверяет полноту результата C-GET.
...
- `patient` — полное ФИО; `age`, `time_beginning` — существующие парсеры из
  `hospital_agent.services.operation_reports`.
- `name_operation` — сокращённое название без номера операционной.
- `study_type` — нормализованный известный тип либо сокращенное название новой операции.
- `department` — сокращение по номеру после `Карта стационарного больного`:
  ...
  Неизвестный код записывается в журнал и не угадывается.
- `descr_operation` — только сокращённое клиническое заключение, без хода
  операции и хирурга.
- `recommendation` — только предложение с фразой «в плановом порядке» либо
  пустая строка.
...
Многолетняя статистика формируется отдельно и не влияет на недельную отправку
протоколов. В начале `scripts\upload_historical_statistics.py` укажите каталог
архива, первый учитываемый год и адрес backend, затем выполните:

```powershell
python scripts\upload_historical_statistics.py
```

Скрипт рекурсивно разбирает DOCX штатным парсером агента, исключает копии одной
операции, группирует результаты по годам и отправляет таблицу в
`PUT /statistics/history`. Неизвестные названия сохраняются в колонке
`ДРУГИЕ`, поэтому ни один успешно распознанный протокол не теряется.

Для первичной безопасной загрузки всех протоколов 2026 года укажите каталог в
начале `scripts\upload_2026_protocols.py`, затем запустите:

```powershell
python scripts\upload_2026_protocols.py
```

Скрипт сначала читает уже загруженные записи с `scope=all`, исключает
